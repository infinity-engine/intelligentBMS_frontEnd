...
export class ShowAllTestsResultComponent implements OnInit, OnDestroy {
  ...
  currentTestResultDeep?:
    | TestResultDeep['Pulse charging']
    | TestResultDeep['Charge/discharge experiment']
    | any;
  ...
  ngOnInit(): void {
    let sbs = this.testData
      .getTestDataLight()
      .subscribe((result: TestResultLight[]) => {
  ...
  setCurrentTestResult(testId: any) {
    ...
    if (testId) {
      this.deepSbs = this.testData
        .getTestDataDeep()
        .subscribe((result: TestResultDeep['Charge/discharge experiment']) => {
